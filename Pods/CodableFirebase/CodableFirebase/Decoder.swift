...
class _FirebaseDecoder: Decoder {
    ...
    init(referencing container: Any, at codingPath: [CodingKey] = [], options: _Options) {
        storage = _FirebaseDecodingStorage()
...
private struct _FirebaseKeyedDecodingContainer<K: CodingKey>: KeyedDecodingContainerProtocol {
    ...
    public func nestedContainer<NestedKey>(keyedBy _: NestedKey.Type, forKey key: Key) throws -> KeyedDecodingContainer<NestedKey> {
        ...
        let container = _FirebaseKeyedDecodingContainer<NestedKey>(referencing: decoder, wrapping: dictionary)
        return KeyedDecodingContainer(container)
    }
    ...
    public func nestedUnkeyedContainer(forKey key: Key) throws -> UnkeyedDecodingContainer {
        ...
        return _FirebaseUnkeyedDecodingContainer(referencing: decoder, wrapping: array)
    }
...
private struct _FirebaseUnkeyedDecodingContainer: UnkeyedDecodingContainer {
    ...
    public mutating func decodeNil() throws -> Bool {
        guard !isAtEnd else {
            throw DecodingError.valueNotFound(Any?.self, DecodingError.Context(codingPath: decoder.codingPath + [_FirebaseKey(index: currentIndex)], debugDescription: "Unkeyed container is at end."))
        }
    ...
    public mutating func decode<T: Decodable>(_ type: T.Type) throws -> T {
        ...
        guard let decoded = try decoder.unbox(container[currentIndex], as: T.self) else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath: decoder.codingPath + [_FirebaseKey(index: currentIndex)], debugDescription: "Expected \(type) but found null instead."))
        }
